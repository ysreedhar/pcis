...
main.lbltitle.Caption = "OBS CODE LIST"
Me.Top = 10
Me.Left = 10
Option1.Value = False
Option2.Value = True
...
ls.Open "select Distinct(resp_code),resp_desc from responsibledetails order by resp_code", Cn, 3, 2
While Not ls.EOF
...
rs.Open "select * from responsibledetails where resp_code='" & nm(0) & "' order by resp_code", Cn, 3, 2
While Not rs.EOF
  cnt = cnt + 1 '********************************
                                If cnt >= 58 Then
...
fs.WriteLine "    <table border=1 cellspacing=0 BORDERCOLOR=gray width=95%>"
   ...
   fs.WriteLine "            <td Nowrap><font color=white>Resp Code</td>"
