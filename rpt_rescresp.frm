...
Begin VB.Form rpt_rescresp 
   ...
   End
End
Attribute VB_Name = "rpt_rescresp"
...
main.lbltitle.Caption = "RESOURCE RESPONSIBILITY CODE LIST"
Me.Top = 10
Me.Left = 10
Option1.Value = False
Option2.Value = True
...
ls.Open "select Distinct(resp_code),resp_desc from responsiblemaster order by resp_code", Cn, 3, 2
While Not ls.EOF
...
rs.Open "select * from responsiblemaster where resp_code='" & nm(0) & "' order by resp_code", Cn, 3, 2
While Not rs.EOF
 cnt = cnt + 1 '********************************
                                If cnt >= 55 Then
...
fs.WriteLine "            <td  >" & rs!resp_code & "</td>"
If rs!resp_desc = "" Then
fs.WriteLine "            <td  >&nbsp;</td>"
Else
fs.WriteLine "            <td  >" & rs!resp_desc & "</td>"
End If
...
fs.WriteLine "    <table border=1 cellspacing=0 BORDERCOLOR=gray width=95%>"
                ...
                fs.WriteLine "           <td  ><b>RESOURCE RESPONSIBLE LIST</td>"
   ...
   fs.WriteLine "            <td Nowrap><font color=white>Resc Responsible</td>"
